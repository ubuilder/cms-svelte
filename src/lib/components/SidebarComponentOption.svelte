    ...
    <div class="ms-auto flex gap-2">
      ...
      {#if !getComponent(activeSlot.type).global}
        <SidebarTitleButton
          icon="settings"
          color="secondary"
          on:click={() => openComponentSettings()} />
      {/if}
    </div>
  </div>
            ...
            {:else if activeSlot.props[field.name]}
              <FormField mb="2">
                <div class="flex items-center justify-between">
                  ...
                  <Switch
                    checked={activeSlot.props[field.name].name}
                    on:change={() => switchDynamic(field.name)} />
                </div>
                    ...
                    <Accordion>
                      ...
                      <AccordionBody style="background-color: var(--y-bg-surface)" p="0">
                        <El p="2">
                          <FormInput
                            label="Name (as)"
                            bind:value={activeSlot.props[field.name].name} />

                          <FormSelect
                            bind:value={activeSlot.props[field.name].table}
                          ...
                          <FormCheckbox
                            label="Multiple"
                            bind:checked={activeSlot.props[field.name].multiple} />
                          <FormField label="Filters">
                            <Accordions>
                              {#each activeSlot.props[field.name].filters ?? [] as filter}
                                ...
                                <Accordion>
                                    ...
                                    <El
                                      ...
                                      <Button
                                        ...
                                        on:click!stopPropagation={() =>
                                          (activeSlot.props[field.name].filters = activeSlot.props[
                                            field.name
                                  ...
                                  <AccordionBody
                                    style="background-color: var(--y-bg-surface)"
                                    p="0">
                                    {#if table}
                                      <El p="2" row>
                                        <FormSelect
                            ...
                            <Button
                              ...
                              color="primary"
                              on:click={() =>
                                (activeSlot.props[field.name].filters = [
                                  ...(activeSlot.props[field.name].filters ?? []),
                                  { field: undefined, operator: '=', value: '' },
                                ])}>
                              <Icon name="plus" />
                              Add New Filter
                            </Button>
                          ...
                          <FormField label="Pagination...">
                            <El row>
                              <FormInput
                                col="6"
                                bind:value={activeSlot.props[field.name].page}
                                label="Page"
                                type="number" />
                              <FormInput
                                col="6"
                                bind:value={activeSlot.props[field.name].perPage}
                                label="Per Page"
                                type="number" />
                            </El>
                          </FormField>
