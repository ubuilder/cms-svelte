<script lang="ts">
  ...
  let items = []
	$: items = components
		.filter((x) => !disabledComponents?.includes(x.id))
		.filter((x) => {
      console.log(x, allowedComponents)
      if(allowedComponents?.length) {
        return allowedComponents.includes(x.id)
      }
			return !x.hidden
...
<BaseModal title={'Choose a component'}>
	...
	<ModalFooter>
		<ButtonList>
			...
			<Button on:click={() => $modal.resolve(slot.type ? slot : undefined)} color="primary"
