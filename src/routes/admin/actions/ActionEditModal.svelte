...
<Modal {title}>
  <El class="action-edit" row slot="body">
    <El col>
      <FormInput required label="Name" bind:value={action.name} />
    </El>
    <El col="12">
                ...
                <El row>
                  ...
                  <El col="12" d="flex" justifyContent="end">
                    <Button
                      ...
                      color="danger"
                      on:click={() =>
                        (action.fields = action.fields.filter(
                          (x) => x !== field
                        ))}
                    >
                      Remove
  ...
  <ButtonList justifyContent="end" slot="footer">
    ...
    <Button disabled={!action.name} color="primary" on:click={() => $modal.resolve(action)}>
      {submitText}
    </Button>
  </ButtonList>
</Modal>
