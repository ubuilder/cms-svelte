...
export const load: ServerLoad = async ({ params, locals }) => {
  const table = await locals
    .db("u-tables")
    .get({ where: { slug: params.slug } });
  ...
  for (let value of rows.data) {
    for (let field of table.fields) {
      if (field.type === "relation") {
        if (field.multiple) {
          console.log(
            "loiad multuple relations: ",
            field.table,
            field.field,
            value.id
          );
          value[field.name] = await locals
            .db(field.table)
            .query({
              where: {
                [field.field + "_id"]: {
  ...
  for (let field of table.fields) {
    ...
    if (body[field.name] && field.type === "relation") {
      if (field.multiple) {
        // user.posts
        // update other table
        const otherTable = await locals
          .db("u-tables")
          .get({ where: { slug: field.table } });
